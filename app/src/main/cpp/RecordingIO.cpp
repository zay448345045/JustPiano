...
void RecordingIO::clearRecordingBuffer() {
    ...
    if (mRecordingFile != -1) {
        ...
        size_t wavFilePathLength = strlen(mRecordingFilePath);
...
void RecordingIO::generateWavFileHeader(char *header, long totalAudioLen, long longSampleRate,
    ...
    long byteRate = longSampleRate * channelCount * sizeof(float);
    ...
    header[32] = (char) (channelCount * sizeof(float));
    header[33] = 0;
    // 每个样本的数据位数
