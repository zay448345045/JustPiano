...
JNIEXPORT void JNICALL
Java_com_mobileer_androidfxlab_NativeInterface_addDefaultEffectNative(JNIEnv
*, jobject,
jint jid
) {
if (!enginePtr) return;
...
stack.
addEffect(std::move(f)
);
}, enginePtr->functionList);
}
JNIEXPORT void JNICALL
Java_com_mobileer_androidfxlab_NativeInterface_removeEffectNative(JNIEnv
*, jobject,
jint jind
) {
if (!enginePtr) return;
...
JNIEXPORT void JNICALL
Java_com_mobileer_androidfxlab_NativeInterface_rotateEffectNative(JNIEnv
*, jobject,
jint jfrom, jint
jto) {
if (!enginePtr) return;
...
env->
ReleaseFloatArrayElements(params, data,
0);
std::visit([&arr, &id, &index](
auto &&stack
) {
...
stack.
modifyEffectAt(index, std::move(ef)
);
}, enginePtr->functionList);
}
JNIEXPORT void JNICALL
...
if (!enginePtr) return;
std::visit([
enable = static_cast<bool>(jenable)
](
auto &&args
) {
args.mute(!enable);
